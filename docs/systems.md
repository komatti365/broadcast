...
また、オプション設定により引用開始時の動画情報を放送者コメントとして表示することもできます。
また、オプション設定を利用することで、引用開始時の動画情報をDiscordに通知する機能も追加しました。

## nucosen.py

最も規模の大きいプログラムです。
環境変数や設定ファイルを読み込み、各プログラムを呼び出して初期情報を与えます。
また、バックグラウンドスレッドを通じて、DB設定の動的反映や、新着ピックアップモードのための動画準備、再生キューの補充などを非同期に行います。
...
該当タグを持つ動画の中からランダムに動画を選出する機能に加え、新着ピックアップモードのための新しい動画の選定や、完全一致タグやカテゴリ・ジャンルタグを用いた詳細な条件での動画検索にも対応しています。
